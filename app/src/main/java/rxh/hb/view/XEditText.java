 ...
 * 使用pattern时无需在xml中设置maxLength属性，若需要设置时应注意加上分隔符的数量 Created by woxingxiao on
 * 2015/9/4.
 */
public class XEditText extends EditText {
	...
	public XEditText(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (separator == null)
			separator = SPACE;
		customizeMarkerEnable = a.getBoolean(
				R.styleable.XEditText_x_customizeMarkerEnable, false);
		...
		iOSStyleEnable = a.getBoolean(R.styleable.XEditText_x_iOSStyleEnable,
				false);
		a.recycle();
	...
	private void init() {
		...
		if (mRightMarkerDrawable == null) { // 如未设置则采用默认
			mRightMarkerDrawable = getResources().getDrawable(
					R.drawable.icon_clear);
			if (mRightMarkerDrawable != null)
				mRightMarkerDrawable.setBounds(0, 0,
						mRightMarkerDrawable.getIntrinsicWidth() / 2,
						mRightMarkerDrawable.getIntrinsicHeight() / 2);
		}
	...
	private void initiOSObjects() {
		...
		if (mHintCharSeq != null) {
			...
			if (mRect == null || mTextRect == null || mTextPaint == null) {
				...
				mTextPaint.getTextBounds(mHintCharSeq.toString(), 0,
						mHintCharSeq.length(), mTextRect);
			}
		}
		iOSFrameHide = false;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (iOSStyleEnable) {
			...
			if (mHintCharSeq != null) {
				Paint.FontMetricsInt fontMetrics = mTextPaint
						.getFontMetricsInt();
				int textCenterY = (mRect.bottom + mRect.top
						- fontMetrics.bottom - fontMetrics.top) / 2;
				canvas.drawText(mHintCharSeq.toString(), canvas.getWidth() / 2,
						canvas.getHeight() / 2 + textCenterY, mTextPaint);
			}
			if (mBitmap != null) {
				canvas.drawBitmap(
						mBitmap,
						(canvas.getWidth() - mTextRect.width()) / 2
								- mBitmap.getWidth()
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (hasFocused && mRightMarkerDrawable != null
			...
			boolean isAreaY = event.getY() >= rectTopY
					&& event.getY() <= (rectTopY + height);
			if (isAreaX && isAreaY) {
				if (customizeMarkerEnable) {
					if (mMarkerClickListener != null)
						mMarkerClickListener.onMarkerClick(event.getRawX(),
								event.getRawY());
				} else {
					setError(null);
					setText("");
	 ...
	 * Marker在什么时间显示
	 * 
	 * @param showMarkerTime
	 *            BEFORE_INPUT：没有输入内容时显示； AFTER_INPUT：有输入内容后显示；
	 *            ALWAYS：（获得焦点后）一直显示
	 */
	...
	/**
	 * @param hasNoSeparator
	 *            true设置无分隔符模式，功能同EditText
	 */
