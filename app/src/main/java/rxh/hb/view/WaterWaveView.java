...
public class WaterWaveView extends View {
	...
	private WaterWaveViewAnimation anim;
	...
	private void initView() {
		...
		mPaintInnerCircle.setColor(Color.parseColor("#ffa876"));
		...
		mPaintWater.setColor(Color.parseColor("#fb6006"));
		mPaintWater.setStyle(Paint.Style.FILL);

		anim = new WaterWaveViewAnimation();
	...
	protected void onDraw(Canvas canvas) {
		...
		// 绘制中间的边界的元环
		// canvas.drawCircle(mCenterXY, mCenterXY, mRadius - dpToPx(2),
		// mPaintBorderCircle);
		...
		// (getHeight() + mRectProgress.height()) / 2,
		// mPaintText);
	...
	private void createWavePath(int progress) {
		...
		for (int i = 0; i < absX * 2; i++) {
			x = i + startX;
			y = (float) (mWaveHeight * Math.sin((i * 1.5f + mOffsetX) / mRadius * Math.PI)) + (mRadius * 2 - absY)
					+ mStrokeWidth;
			if (i == 0) {
				mPath.moveTo(x, y);
			} else {
	...
	// 动画
	public class WaterWaveViewAnimation extends Animation {
		@Override
		protected void applyTransformation(float interpolatedTime, Transformation t) {
			super.applyTransformation(interpolatedTime, t);
			progress = (int) (f_progress * (interpolatedTime * 100));
			postInvalidate();
		}
	}
